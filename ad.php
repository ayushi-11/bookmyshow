...
$sql = "select distinct sum(concerts.price) from concerts,book where Eid=Cid;";
$result = $conn->query($sql);

if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
    $consum=$row["sum(concerts.price)"];
    }
} else {
    echo "0 results";
}
$sql = "select distinct sum(Tprice) from theatre,book where Eid=Tid;";
$result = $conn->query($sql);

if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
    $thesum=$row["sum(Tprice)"];
    }
} else {
    echo "0 results";
}
$sql = "select distinct sum(price) from movies,book where Eid=Mid;";
$result = $conn->query($sql);

if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
    $moviesum=$row["sum(price)"];
    }
} else {
    echo "0 results";
}
...
<div class="bg-top">
<div class="shell">
<h1> HELLO ADMIN (updated report)</h1><br><br>
<br>
<div id="form">
<p>net booking for concerts <?php echo ("$consum");?></p>
<p>net booking for theatre <?php echo ("$thesum");?></p>
<p>net booking for movies <?php echo ("$moviesum");?></p>
</div>
</form>
</div>
</div>
